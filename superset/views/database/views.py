...
from superset.utils.s3_helper import AwsS3Helper
...
def parquet_to_csv_bytesio(parquet_bytesio):
    """
    Converts a Parquet BytesIO object to a CSV BytesIO object.

    Args:
    parquet_bytesio: A BytesIO object containing Parquet data.

    Returns:
    A BytesIO object containing CSV data.
    """

    df = pd.read_parquet(parquet_bytesio)
...
class S3ToDatabaseView(CsvToDatabaseView):
    form = S3ToDatabaseForm
    form_title = _("Import from AWS S3")

    def form_post(self, form: CsvToDatabaseForm) -> Response:
        database = form.database.data
        csv_table = Table(table=form.table_name.data, schema=form.schema.data)
        delimiter_input = form.delimiter.data
        print('UPLOADING...', form.__dict__)
        ...
        if not schema_allows_file_upload(database, csv_table.schema):
            ...
            flash(message, "danger")
            return redirect("/s3todatabaseview/form")
        ...
        object_key = s3_helper.convert_filename_to_key(form.csv_file.data)
        print('OBJECTKEY', object_key)
        parquet_bytesio = s3_helper.fetch_file_using_key(config["S3_BUCKET"], object_key)
        csv_bytesio = parquet_to_csv_bytesio(parquet_bytesio)
        # except Exception as ex:
        #     message = _(
        #         'Unable to upload CSV file "%(filename)s". '
        #         "Error message: %(error_msg)s",
        #         filename=form.csv_file.data,
        #         error_msg=str(ex),
        #     )
        #     flash(message, "danger")
        #     return redirect("/s3todatabaseview/form")
        ...
        try:
            ...
            df = pd.concat(
                pd.read_csv(
                    chunksize=1000,
                    encoding="utf-8",
                    filepath_or_buffer=csv_bytesio,
                    header=form.header.data if form.header.data else 0,
                    index_col=form.index_col.data,
                    infer_datetime_format=form.infer_datetime_format.data,
                    iterator=True,
        ...
        message = _(
            'CSV file "%(csv_filename)s" uploaded to table "%(table_name)s" in '
            'database "%(db_name)s"',
            csv_filename=form.csv_file.data,
            table_name=str(csv_table),
            db_name=sqla_table.database.database_name,
        )
        flash(message, "info")
