...
def is_allowed_file(filename):
    return filename.endswith('.parquet') or filename.endswith('.csv')


class S3FileExplorerForm(DynamicForm):

    @staticmethod
    def file_allowed_s3() -> list[str]:
        s3_helper = AwsS3Helper()
        files = s3_helper.list_all_files(
            config["S3_BUCKET"],
        )
        ...
        for f in files:
            ...
            if is_allowed_file(filename):
                modified_filename = s3_helper.convert_key_to_filename(filename)
                processed_files.append(modified_filename)
...
class S3ToDatabaseForm(UploadToDatabaseForm):
    csv_file = SelectField(
        _("File"),
        description=_("Select a file to import"),
        choices=S3FileExplorerForm.file_allowed_s3(),
        validators=[DataRequired()],
    )
    table_name = StringField(
        _("Table Name"),
